...
# ========== 5. VẼ BIỂU ĐỒ ĐẸP ==========
def plot_duval_triangle(ch4, c2h4, c2h2, fault=None):
    total = ch4 + c2h4 + c2h2
    if total <= 0:
        return None
    pCH4 = ch4 / total * 100
    ...
    for zone, path in PATHS_T1.items():
        ...
        ax.text(centroid[0], centroid[1], zone_short_labels.get(zone, zone),
                ...
                color=_text_color_for_bg(color), fontsize=9)

    # Lưới % (đường ngang và xiên)
    ...
    # Mũi tên và nhãn ngoài tam giác
    ...
    def get_normal(p1, p2):
        """Pháp tuyến ngoài (đơn vị) cho cạnh từ p1->p2."""
        if np.allclose(p1, [0,0]) and np.allclose(p2, [0.5, h]):
            n = np.array([-h, 0.5])
        elif np.allclose(p1, [0.5, h]) and np.allclose(p2, [1,0]):
            n = np.array([h, 0.5])
        elif np.allclose(p1, [0,0]) and np.allclose(p2, [1,0]):
            n = np.array([0, -1])
        else:
            n = np.array([0, 1])
        return n / np.linalg.norm(n)

    def draw_edge_arrow(p1, p2, direction, label):
        v = np.array(p2) - np.array(p1)
        uv = v / np.linalg.norm(v)
        n = get_normal(p1, p2)
        mid = (np.array(p1) + np.array(p2)) / 2.0
        center = mid + n * offset_arrow
        start = center - uv * (arrow_length / 2) * direction
        end = center + uv * (arrow_length / 2) * direction
        arrow = FancyArrowPatch(start, end, arrowstyle=arrow_style, color='black', lw=1.5)
        ax.add_patch(arrow)
        label_pos = mid + n * offset_label
        ax.text(label_pos[0], label_pos[1], label, fontsize=12, fontweight='bold',
                ha='center', va='center')

    draw_edge_arrow([0,0], [0.5, h], direction=1, label='CH₄')
    draw_edge_arrow([0.5, h], [1,0], direction=1, label='C₂H₄')
    draw_edge_arrow([0,0], [1,0], direction=-1, label='C₂H₂')

    # Nhãn góc
    ax.text(0.5, h + 0.03, 'CH₄', ha='center', va='bottom', fontsize=13, fontweight='bold')
    ax.text(-0.05, -0.02, 'C₂H₂', ha='right', va='top', fontsize=13, fontweight='bold')
    ax.text(1.05, -0.02, 'C₂H₄', ha='left', va='top', fontsize=13, fontweight='bold')

    # Điểm mẫu
    ax.scatter(*sample_xy, color='red', marker='X', s=140, zorder=10,
               edgecolors='black', linewidths=1.5)
    label_text = fault if fault else f"CH₄ {pCH4:.1f}%\nC₂H₄ {pC2H4:.1f}%\nC₂H₂ {pC2H2:.1f}%"
    ax.annotate(label_text, xy=sample_xy, xytext=(sample_xy[0]+0.12, sample_xy[1]+0.08),
    ...
    # Tiêu đề
    fault_full = fault_explanations.get(fault, '') if fault else ''
    ax.set_title(f"Duval Triangle 1 – Fault: {fault}\n{fault_full}",
