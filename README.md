...
Next.js dashboard visualizes the results â€” fleet overview, analytics, per-transformer
detail, and an AI chat assistant â€” behind a login.
...
- `backend/app.py` â€” the Flask server: `/auth/*`, `/predict`, `/chat`, `/dataset/reset`, `/chart/*`
- `backend/inference_service.py` â€” orchestrates clean â†’ accumulate â†’ feature-engineer â†’ diagnose â†’ score â†’ rank
- `backend/dataset_accumulator.py` â€” merges each new upload into everything uploaded
  before (`backend/data/accumulated_clean.csv`), deduped by transformer + sample date
- `backend/dga/` â€” the six traditional diagnostic methods (Duval Triangle, Duval Pentagon,
  IEC 60599, Rogers Ratio, Doernenburg, Key Gas)
- `backend/consensus.py`, `backend/severity.py`, `backend/ranking.py` â€” combine the six
  methods' votes into a consensus fault, score severity, and rank the fleet
- `backend/train_models.py` â€” offline training for the LightGBM/XGBoost/PyTorch models
  (not required to run the live dashboard â€” the pipeline above is rule-based)
- `backend/auth.py` â€” local SQLite-backed single-user login (no self-service registration)
- `backend/data_store.py` â€” mirrors every `/predict` result into SQLite (`backend/data/dga.db`)
  so the chatbot has something to query
- `backend/text2sql_chat.py` â€” the DGA Assistant: turns a question into SQL against that
...
No trained model is required to use the dashboard â€” `/predict` runs the rule-based
...
browser (not through a Next.js proxy â€” large uploads can take close to a minute, and
...
The dashboard is behind a login, but there's no self-service registration â€” it's a
...
`seed_user.py` any time to change the email/password â€” it replaces whichever
...
several no-cost ":free" models â€” no payment method required):
...
Without a key, `/chat` still works â€” it silently falls back to a smaller rule-based
...
CSV/XLSX file â€” prediction runs automatically as soon as it's selected, no extra
...
dataset (six diagnostic methods Ã— severity scoring Ã— fleet ranking)
can take up to a minute â€” the panel shows a running timer so it's clear it's still
working, not stuck.

Uploads **accumulate** rather than replace: each new file is merged with everything
uploaded before (deduped by transformer + sample date â€” the newest upload wins on an
exact-date conflict), then the *entire* combined dataset is re-cleaned, re-scored, and
re-ranked. Different files can have different columns/formats â€” each is normalized by
...
From there:
- **Overview** (`/`) â€” fleet-wide status counts and the risk-ranked table
- **Analytics** (`/analytics`) â€” severity/fault-type distribution, top-5-risk trend
- **Fleet** (`/fleet`) â€” transformer directory grouped by station
- **Transformer detail** (`/transformer/:id`) â€” gas indicators, a severity trend chart
  ...
  method's individual vote for that record â€” plus field inspection confirmation
- **DGA Assistant** (floating button, resizable and remembers conversation context
  within the session) â€” asks the backend's `/chat` endpoint questions
