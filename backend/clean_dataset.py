...
def _prepare_dataframe(
    ...
    """
    Prepare an already-loaded DataFrame.
    ...
    # ------------------------------------------------------------
    # Normal case:
    # Flask already parsed the file and supplied column headers.
    # ------------------------------------------------------------
    rename_map = {}
    ...
    # ------------------------------------------------------------
    # Some uploaded Excel files may already contain a raw header
    # row as normal data. Detect that case only when sample_day
    # is absent.
    # ------------------------------------------------------------
...
def clean_dataset(
    ...
    Using dataframe avoids the expensive:
        DataFrame -> XLSX -> read_excel -> DataFrame
    round trip.
    """
    ...
    inferred_loc = df.apply(
        infer_loc_from_codetx_and_name,
        axis=1,
    )

    df["loc"] = df["loc"].fillna(
        inferred_loc
    )
