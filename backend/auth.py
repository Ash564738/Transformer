...
1. Local / traditional mode: uses backend/database/users.db, compatible with seed_user.py, sessions stored in SQLite.
2. Stateless deployment mode: activated automatically when DGA_AUTH_SECRET is set, no users.db or writable filesystem required.
   The single configured account is read from environment variables:
       DGA_ADMIN_EMAIL, DGA_ADMIN_PASSWORD, DGA_ADMIN_NAME, DGA_AUTH_SECRET, DGA_SESSION_HOURS
...
ADMIN_EMAIL = (os.getenv("DGA_ADMIN_EMAIL") or os.getenv("ADMIN_EMAIL") or "").strip().lower()
ADMIN_PASSWORD = (os.getenv("DGA_ADMIN_PASSWORD") or os.getenv("ADMIN_PASSWORD") or "")
ADMIN_NAME = (os.getenv("DGA_ADMIN_NAME") or os.getenv("ADMIN_NAME") or "Administrator").strip()
try:
    SESSION_HOURS = max(1, int(os.getenv("DGA_SESSION_HOURS", "24")))
except ValueError:
    SESSION_HOURS = 24
...
def init_db():
    """Initialize the SQLite authentication database. In stateless deployment mode there is deliberately no database initialization because Vercel filesystem state should not be used as persistent authentication storage."""
...
def _validate_stateless_config() -> None:
    ...
    if not AUTH_SECRET:
        raise RuntimeError("DGA_AUTH_SECRET is not configured.")
    if not ADMIN_EMAIL:
        raise RuntimeError("DGA_ADMIN_EMAIL is not configured.")
    if not EMAIL_RE.match(ADMIN_EMAIL):
        raise RuntimeError("DGA_ADMIN_EMAIL is not a valid email address.")
    if not ADMIN_PASSWORD:
        raise RuntimeError("DGA_ADMIN_PASSWORD is not configured.")
    if not ADMIN_NAME:
        raise RuntimeError("DGA_ADMIN_NAME is not configured.")
...
def _serializer() -> URLSafeTimedSerializer:
    ...
    _validate_stateless_config()
    return URLSafeTimedSerializer(AUTH_SECRET, salt="dga-dashboard-auth-v1")
...
def set_single_user(email: str, password: str, name: str) -> dict:
    """Replace the local SQLite account with exactly one account. This function is intentionally disabled in stateless deployment mode."""
    if is_stateless_mode():
        raise RuntimeError("set_single_user() is disabled in stateless deployment mode. Configure DGA_ADMIN_EMAIL, DGA_ADMIN_PASSWORD and DGA_ADMIN_NAME instead.")
...
def _create_stateless_session(user: dict) -> str:
    ...
    payload = {"type": "dga-session", "user": user}
    signed = serializer.dumps(payload)
    return f"stateless.{signed}"

def create_session(user_id: int) -> str:
    """Create a local SQLite session. In stateless mode, user_id is ignored and the configured deployment account is used."""
    if is_stateless_mode():
...
# Login
# ---------------------------------------------------------------------------
def _stateless_login(email: str, password: str) -> tuple[dict, str]:
    """Authenticate against deployment environment variables."""
    user = _stateless_user()
    normalized_email = (email or "").strip().lower()
    if normalized_email != user["email"]:
        raise ValueError("Incorrect email or password.")
    if not secrets.compare_digest(password or "", ADMIN_PASSWORD):
        raise ValueError("Incorrect email or password.")
    token = _create_stateless_session(user)
    return user, token

def _sqlite_login(email: str, password: str) -> tuple[dict, str]:
...
def _user_from_stateless_token(token: str) -> dict | None:
    ...
    try:
        serializer = _serializer()
        payload = serializer.loads(encoded, max_age=_max_age_seconds())
    ...
    if user.get("email") != ADMIN_EMAIL:
        return None
    return {"id": user.get("id", 1), "email": user["email"], "name": user.get("name", ADMIN_NAME)}
...
def logout(token: str) -> None:
    """Logout. Stateless tokens cannot be revoked individually because there is no persistent session store. They expire automatically after DGA_SESSION_HOURS. Local SQLite sessions are explicitly deleted."""
    if is_stateless_mode():
        return
    if not token:
        return
