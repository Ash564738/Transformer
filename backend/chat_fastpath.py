...
DGA_METHOD_ALIASES = {
    "key gas": "Key Gas",
    "keygas": "Key Gas",
    "key gas method": "Key Gas",
    "iec 60599": "IEC 60599",
    "iec60599": "IEC 60599",
    "iec": "IEC 60599",
    "rogers ratio": "Rogers Ratio",
    "rogers": "Rogers Ratio",
    "doernenburg": "Doernenburg",
    "dörnenburg": "Doernenburg",
    "dorn": "Doernenburg",
    "duval triangle 1": "Duval Triangle",
    "duval triangle": "Duval Triangle",
    "triangle": "Duval Triangle",
    "duval pentagon": "Duval Pentagon",
    "pentagon": "Duval Pentagon",
}
...
def extract_requested_dga_methods(
    question: str,
) -> list[tuple[str, str]]:
    ...
    for alias, method_name in sorted(
        ...
        if alias in text and method_name not in seen:
            ...
            found.append(
                (method_name, ALL_DGA_METHODS[method_name])
            )
    ...
    if _contains_any(text, all_terms):
        return list(ALL_DGA_METHODS.items())
...
TRANSFORMER_ID_PATTERNS = [
    re.compile(
        r"\b(?:máy|transformer|tx|tr)\s*#?\s*([A-Za-z0-9_-]+)",
        re.IGNORECASE,
    ),
    re.compile(
        r"\b(?:transformer_id|transformer|máy)\s*[=:]\s*['\"]?([A-Za-z0-9_-]+)",
        re.IGNORECASE,
    ),
    re.compile(
        r"\b(?:เครื่อง|หม้อแปลง)\s*#?\s*([A-Za-z0-9_-]+)",
        re.IGNORECASE,
    ),
]
...
def _open_ro():
    if not DB_PATH.exists():
        raise FileNotFoundError("No DGA dataset loaded.")
...
def fast_domain_answer(question: str, language: str) -> str | None:
    ...
    if _asks_comparison(question) and len(methods) >= 2:
        if language == "vi":
            lines = ["So sánh các phương pháp DGA:"]
        elif language == "th":
            lines = ["เปรียบเทียบวิธีวิเคราะห์ DGA:"]
        else:
            lines = ["Comparison of the requested DGA methods:"]

        for method, _ in methods:
            lines.append(
                f"- {method}: {_method_knowledge(method, language)}"
            )

        if language == "vi":
            lines.append(
                "Không có một phương pháp duy nhất luôn tốt nhất. "
                "Trong thực tế nên đối chiếu nhiều phương pháp vì chúng sử dụng "
                "các tập khí và cách biểu diễn khác nhau."
            )
        elif language == "th":
            lines.append(
                "ไม่มีวิธีใดวิธีหนึ่งที่ดีที่สุดเสมอไป "
                "ควรพิจารณาหลายวิธีร่วมกัน เพราะแต่ละวิธีใช้ข้อมูลก๊าซและรูปแบบการวินิจฉัยต่างกัน"
            )
        else:
            lines.append(
                "No single method is always the best. "
                "Multiple methods should be compared because they use different gas information and diagnostic representations."
            )
        return "\n".join(lines)
    ...
    if len(methods) == 1 and _asks_types(question):
        ...
        if method == "Duval Triangle":
            answers = {
                "vi": "Duval Triangle 1 có 6 fault type chính: PD, D1, D2, T1, T2 và T3.",
                "th": "Duval Triangle 1 มี fault type หลัก 6 ประเภท: PD, D1, D2, T1, T2 และ T3",
                "en": "Duval Triangle 1 has six main fault types: PD, D1, D2, T1, T2 and T3.",
            }
            return answers[language]
        return _method_knowledge(method, language)

    if len(methods) == 1 and _contains_any(
        question,
        [
            "là gì",
            "nghĩa là gì",
    ...
    if "tdcg" in lowered and _contains_any(
        question,
        [
            "là gì",
            "nghĩa là gì",
    ...
    if _contains_any(
        ...
        if language == "vi":
            return f"Backend hiện sử dụng 6 phương pháp DGA: {names}."
        if language == "th":
            return f"ระบบนี้ใช้วิธี DGA 6 วิธี ได้แก่ {names}"
...
def _fast_count_critical(question: str, language: str) -> str | None:
    if not _contains_any(
        ...
        [
            ...
            "count critical transformers",
            "có bao nhiêu máy critical",
            "bao nhiêu máy đang critical",
            "critical กี่เครื่อง",
        ],
    ):
        return None
    ...
    if language == "vi":
        return f"Hiện có {count} máy biến áp ở trạng thái Critical."
    if language == "th":
        return f"ขณะนี้มีหม้อแปลงสถานะ Critical จำนวน {count} เครื่อง"
...
def _fast_most_critical(question: str, language: str) -> str | None:
    if not _contains_any(
        question,
        [
            "most critical transformer",
            "highest risk transformer",
            "highest priority transformer",
            "máy nào nghiêm trọng nhất",
            "máy biến áp nào nghiêm trọng nhất",
            "máy nào rủi ro cao nhất",
            "เครื่องไหนรุนแรงที่สุด",
            "หม้อแปลงตัวไหนเสี่ยงสูงสุด",
        ],
    ):
        return None

    columns, rows = _query(
        "SELECT transformer_id,loc,status,fault_type,priority_score,recommended_action "
        "FROM transformers ORDER BY priority_score DESC LIMIT 1"
    )
    ...
    if language == "vi":
        return (
            f"Máy có mức ưu tiên cao nhất hiện tại là {record['transformer_id']}; "
            f"status={record['status']}, fault={record['fault_type']}, "
            f"priority_score={_fmt(record['priority_score'])}. "
            f"Khuyến nghị: {record.get('recommended_action') or 'N/A'}"
        )

    if language == "th":
        return (
            f"หม้อแปลงที่มีความเสี่ยงสูงสุดคือ {record['transformer_id']}; "
            f"status={record['status']}, fault={record['fault_type']}, "
            f"priority_score={_fmt(record['priority_score'])}. "
            f"คำแนะนำ: {record.get('recommended_action') or 'N/A'}"
        )

    return (
        f"The highest-priority transformer is {record['transformer_id']}; "
        f"status={record['status']}, fault={record['fault_type']}, "
        f"priority_score={_fmt(record['priority_score'])}. "
        f"Recommendation: {record.get('recommended_action') or 'N/A'}"
    )
...
def _fast_compare_transformers(question: str, language: str) -> str | None:
    ...
    sql = (
        "SELECT transformer_id,loc,status,fault_type,priority_score,trend,recommended_action "
        f"FROM transformers WHERE transformer_id IN ({placeholders}) "
    ...
    if language == "vi":
        lines = ["So sánh các máy:"]
    elif language == "th":
        lines = ["ผลการเปรียบเทียบหม้อแปลง:"]
    else:
        lines = ["Transformer comparison:"]

    for record in records:
        lines.append(
            f"- {record['transformer_id']}: "
            f"status={record['status']}, "
            f"fault={record['fault_type']}, "
            f"priority={_fmt(record['priority_score'])}, "
            f"trend={record['trend']}, "
            f"action={record['recommended_action'] or 'N/A'}"
        )
...
def _fast_highest_gas(question: str, language: str) -> str | None:
    ...
    rows = _query(
        f"SELECT sample_day,{','.join(columns)} "
        "FROM samples WHERE transformer_id=? ORDER BY sample_day ASC",
        (transformer_id,),
    )[1]

    if not rows:
        return (
            f"Không tìm thấy dữ liệu cho máy {transformer_id}."
            if language == "vi"
            else f"No data was found for transformer {transformer_id}."
            if language == "en"
            else f"ไม่พบข้อมูลของหม้อแปลง {transformer_id}"
        )
    ...
    if language == "vi":
        lines = [
            f"Máy {transformer_id}: khí có nồng độ cao nhất trong toàn bộ lịch sử là "
            f"{gases[overall_gas]} ({_fmt(overall_value)} ppm).",
            "Theo từng sample:",
        ]
    elif language == "th":
        lines = [
            f"หม้อแปลง {transformer_id}: ก๊าซที่มีความเข้มข้นสูงสุดในประวัติคือ "
            f"{gases[overall_gas]} ({_fmt(overall_value)} ppm)",
            "แยกตาม sample:",
        ]
    else:
        lines = [
            f"Transformer {transformer_id}: the highest recorded gas concentration is "
            f"{gases[overall_gas]} ({_fmt(overall_value)} ppm).",
            "By sample:",
        ]
    ...
    for day, gas, value in samples:
        lines.append(
            f"- {day}: {gases[gas]}={_fmt(value)} ppm"
        )
...
def _fast_dga_method_history(question: str, language: str) -> str | None:
    ...
    if language == "vi":
        lines = [
            f"Kết quả DGA của máy {transformer_id}:",
            "Phương pháp: " + ", ".join(x[0] for x in methods),
        ]
    elif language == "th":
        lines = [
            f"ผล DGA ของหม้อแปลง {transformer_id}:",
            "วิธีวิเคราะห์: " + ", ".join(x[0] for x in methods),
        ]
    else:
        lines = [
            f"DGA results for transformer {transformer_id}:",
            "Methods: " + ", ".join(x[0] for x in methods),
        ]
    ...
    for row in rows:
        ...
        for method, column in methods:
            ...
            parts.append(
                f"{method}={value if value not in (None, '') else 'ABSTAIN'}"
            )

        lines.append(
            f"- {record['sample_day']}: " + "; ".join(parts)
        )
...
def _fast_fault_history(question: str, language: str) -> str | None:
    ...
    if not _contains_any(
        question,
        ["lỗi", "fault", "failure", "diagnosis", "diagnostic"],
    ):
        return None
    ...
    if not (asks_frequency or asks_timeline or asks_why):
        return None
    ...
    if not rows:
        if language == "vi":
            return f"Không tìm thấy dữ liệu cho máy {transformer_id}."
        if language == "th":
            return f"ไม่พบข้อมูลของหม้อแปลง {transformer_id}"
        return f"No data was found for transformer {transformer_id}."
    ...
    if language == "vi":
        lines = [
            f"Máy {transformer_id} có lỗi xuất hiện nhiều nhất là "
            f"{top['consensus_fault']} ({top['sample_count']}/{total} sample)."
        ]
    elif language == "th":
        lines = [
            f"หม้อแปลง {transformer_id} พบ fault ที่เกิดบ่อยที่สุดคือ "
            f"{top['consensus_fault']} ({top['sample_count']}/{total} samples)"
        ]
    else:
        lines = [
            f"Transformer {transformer_id} has the most frequent fault "
            f"{top['consensus_fault']} ({top['sample_count']}/{total} samples)."
        ]
    ...
    if asks_why:
        if language == "vi":
            lines.append(
                f"Severity trung bình của fault này là {_fmt(top['avg_severity'])}, "
                f"cao nhất {_fmt(top['max_severity'])}."
            )
        elif language == "th":
            lines.append(
                f"severity เฉลี่ยของ fault นี้คือ {_fmt(top['avg_severity'])} "
                f"และค่าสูงสุด {_fmt(top['max_severity'])}"
            )
        else:
            lines.append(
                f"The average severity for this fault is {_fmt(top['avg_severity'])}, "
                f"with a maximum of {_fmt(top['max_severity'])}."
            )

    if asks_timeline:
        if language == "vi":
            lines.append("Timeline từng sample:")
        elif language == "th":
            lines.append("ลำดับ sample ตามเวลา:")
        else:
            lines.append("Sample-by-sample timeline:")

        for record in samples:
            lines.append(
                f"- {record['sample_day']}: "
                f"fault={record['consensus_fault'] or 'ABSTAIN'}, "
                f"student={record['student_fault_label'] or 'N/A'}, "
                f"severity={_fmt(record['severity_score'])}, "
                f"IEEE={record['ieee_dga_status_label'] or 'N/A'}, "
                f"H2={_fmt(record['h2'])}, "
                f"C2H2={_fmt(record['c2h2'])}, "
                f"TDCG={_fmt(record['tdcg'])}"
            )
...
def fast_path(
    ...
    handlers = (
        _fast_highest_gas,
        _fast_fault_history,
        _fast_count_critical,
        _fast_most_critical,
        _fast_compare_transformers,
        _fast_dga_method_history,
    )

    for handler in handlers:
        try:
            result = handler(question, language)
        except Exception:
            continue
