...
def _resolve_status(row: dict) -> str:
    for key in ["ieee_dga_status_label", "severity_label_text", "severity_label"]:
        value = row.get(key)
        if value is None: continue
        text = str(value).upper()
        if text == "NORMAL": return "Normal"
        if text == "WATCHLIST": return "Watch"
        if text == "WARNING": return "High"
        if text == "CRITICAL": return "Critical"
    try:
        score = float(row.get("severity_score", 0.0))
    except (TypeError, ValueError):
        return "Unknown"
    if score < 33.333: return "Normal"
    if score < 66.666: return "Watch"
    return "Critical"
...
SAMPLE_COLUMNS = [
    ...
    "consensus_fault", "consensus_fault_traditional", "mixed_components",
...
TRANSFORMER_COLUMNS = [
    ...
    "status", "severity", "fault_type", "priority_score", "recommended_action",
...
SAMPLE_TEXT_COLS = {
    ...
    "mixed_components", "diagnostic_coverage", "keygas_fault", "iec_fault",
    "rogers_fault", "doernenburg_fault", "duval_triangle_fault",
    "duval_pentagon_p1_fault", "duval_pentagon_p2_fault", "fault_p1", "fault_p2",
    "student_fault_label", "recommended_action",
}
    ...
    try:
        ...
        for row in payload.get("rows", []):
            sample_rows.append(tuple(
                (_resolve_status(row) if column == "status" else _to_sql_value(row.get(column)))
                for column in SAMPLE_COLUMNS
            ))
        if sample_rows:
            placeholders = ", ".join(["?"] * len(SAMPLE_COLUMNS))
            conn.executemany(f"INSERT INTO samples VALUES ({placeholders})", sample_rows)
        logger.info("Inserted %d samples.", len(sample_rows))
        ...
        for item in payload.get("transformer_summary", []):
            ...
            record = {
                ...
                "loc": item.get("loc"),
                "name": item.get("name"),
                "latest_sample_day": item.get("latest_sample_day"),
                "latest_score": item.get("latest_score"),
                "status": _resolve_status({"severity": item.get("severity"), "severity_score": item.get("latest_score")}),
                "severity": item.get("severity"),
                "fault_type": item.get("fault_type"),
                "priority_score": item.get("priority_score"),
                "recommended_action": item.get("recommended_action"),
                "ieee_dga_status": item.get("ieee_dga_status") or breakdown.get("ieee_dga_status"),
                "ieee_dga_status_label": item.get("ieee_dga_status_label") or breakdown.get("ieee_dga_status_label"),
                "diagnostic_confidence": item.get("diagnostic_confidence", breakdown.get("diagnostic_confidence")),
                "anomaly_percentile": item.get("anomaly_percentile") or breakdown.get("anomaly_percentile"),
                "trend_slope": item.get("trend_slope") or breakdown.get("trend_slope"),
            }
        ...
        logger.info("Inserted %d transformers.", len(transformer_rows))
        conn.commit()
    except Exception:
        conn.rollback()
