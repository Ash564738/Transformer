...
def _numeric_tdcg(values):
    ...
    if not all(value is not None for value in values_to_sum): return np.nan
...
def _compute_delta(current,previous,section):
    ...
    for gas in SEVERITY_GASES:
        ...
        if abs(delta) > limit: exceeding.append(gas); numeric_count += 1
    return {"deltas":deltas,"exceeding":exceeding,"available":True,"max_ratio":max([r for r in ratios if r > 1],default=1.0),"numeric_exceeding_count":numeric_count,"any_exceeding":bool(exceeding),"any_increase_only":any_increase_only}
...
def _window_all_gases_below_table1(window,section):
    for _, row in window.iterrows():
        ...
        if not _complete(values): return False
        age = _safe_age(row); age_bucket = _age_bucket(age); limits = IEEE_TABLE_1[section][age_bucket]
        for gas in SEVERITY_GASES:
            value = values[gas]
            if value is None: return False
            if value >= float(limits[gas]): return False
    return True

def _linear_slope(days,values):
    if len(days) < 3: return np.nan
    if np.ptp(days) <= 0: return np.nan
...
def _compute_rate(group,current_position,section):
    ...
    if not candidates: return {"rates":{},"window":None,"exceeding":[],"available":False,"max_ratio":1.0,"numeric_exceeding_count":0,"any_increase_only":False,"all_window_points_below_table1":False}
    ...
    if selected_window is None: return {"rates":{},"window":None,"exceeding":[],"available":False,"max_ratio":1.0,"numeric_exceeding_count":0,"any_increase_only":False,"all_window_points_below_table1":False}
    ...
    for gas in SEVERITY_GASES:
        ...
        if np.isfinite(ratio): ratios.append(ratio)
        if slope > limit: exceeding.append(gas); numeric_exceeding_count += 1
    return {"rates":rates,"window":{"points":int(selected_point_count),"span_months":float(selected_span_months)},"exceeding":exceeding,"available":True,"max_ratio":max([r for r in ratios if r > 1],default=1.0),"numeric_exceeding_count":numeric_exceeding_count,"any_increase_only":any_increase_only,"all_window_points_below_table1":True}
...
def apply_severity(df, nei_reference=None):
    ...
    if missing: raise ValueError("IEEE C57.104-2019 severity requires columns: {missing}".format(missing=missing))
    out = df.copy()
...
def fit_nei_reference(df): return None
