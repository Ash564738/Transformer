...
def _safe_ratio(a,b):
    ...
    return np.nan if x is None or y is None or y <= 0 else x/y
...
def _gas_snapshot(row):
    return {gas:_positive(row.get(gas,np.nan)) for gas in SEVERITY_GASES}

def _complete(values):
    return all(values.get(gas) is not None for gas in SEVERITY_GASES)
...
def _numeric_tdcg(values):
    ...
    if not all(value is not None for value in values_to_sum):
        return np.nan
    return float(sum(values_to_sum))
...
def _compute_delta(current,previous,section):
    ...
    for gas in SEVERITY_GASES:
        ...
        if np.isfinite(ratio):
            ...
            if ratio > 1: ratios.append(ratio)
            if delta > 0:
                increase_ratios_all.append(ratio)
                if ratio > 1:
                    increase_ratios.append(ratio)
        if abs(delta) > limit:
            exceeding.append(gas); numeric_count += 1
    return {"deltas":deltas,"exceeding":exceeding,"available":True,"max_ratio":max([r for r in ratios if r > 1],default=1.0),"max_ratio_all":max(ratios_all,default=np.nan),"max_increase_ratio":max([r for r in increase_ratios if r > 1],default=1.0),"max_increase_ratio_all":max(increase_ratios_all,default=np.nan),"numeric_exceeding_count":numeric_count,"any_exceeding":bool(exceeding),"any_increase_only":any_increase_only}
...
def _compute_rate(group,current_position,section):
    ...
    if not candidates:
        return {"rates":{},"window":None,"exceeding":[],"available":False,"max_ratio":1.0,"max_ratio_all":np.nan,"numeric_exceeding_count":0,"any_increase_only":False,"all_window_points_below_table1":False}
    ...
    rates={}; exceeding=[]; ratios=[]; ratios_all=[]; numeric_exceeding_count=0; any_increase_only=False
    for gas in SEVERITY_GASES:
    ...
    return {"rates":rates,"window":{"points":int(selected_point_count),"span_months":float(selected_span_months)},"exceeding":exceeding,"available":True,"max_ratio":max([r for r in ratios if r > 1],default=1.0),"max_ratio_all":max(ratios_all,default=np.nan),"numeric_exceeding_count":numeric_exceeding_count,"any_increase_only":any_increase_only,"all_window_points_below_table1":True}
...
def _evaluate_row(row,history,position):
    ...
    # Do NOT collapse concentration and gassing-rate ratios into one scalar.
    # IEEE C57.104 uses Tables 1/2 (absolute concentration), Table 3 (delta),
    # and Table 4 (multi-point rate) as separate evidence channels. Taking the
    # maximum across those channels creates a misleading "severity score" and
    # can make a rate ratio arbitrarily dominate the display.
    concentration_status3_exceedance = max(
        evidence["table2_max_ratio"], evidence["table1_max_ratio"], 1.0
    )
    concentration_exceedance = max(
        evidence["table2_max_ratio"], evidence["table1_max_ratio"], 1.0
    )
    delta_exceedance = float(evidence["table3_max_ratio"]) if np.isfinite(evidence["table3_max_ratio"]) else 1.0
    rate_exceedance = float(evidence["table4_max_ratio"]) if np.isfinite(evidence["table4_max_ratio"]) else 1.0

    # Backward-compatible field: concentration-only evidence. This is NOT a
    # severity score and is deliberately never populated from Table 4 rate.
    max_standardized_exceedance = concentration_exceedance
    # Continuous, per-sample evidence for visualization only.  These ratios are
    # normalized directly against the corresponding IEEE reference thresholds.
    # They are NOT a weighted severity score, failure probability, or health index.
    table1_all_ratio_values = [x for x in evidence.get("table1_ratios", {}).values() if np.isfinite(x)]
    table2_all_ratio_values = [x for x in evidence.get("table2_ratios", {}).values() if np.isfinite(x)]
    table1_concentration_ratio_all = max(table1_all_ratio_values, default=np.nan)
    table2_concentration_ratio_all = max(table2_all_ratio_values, default=np.nan)
    # For deterioration visualization, only positive delta is treated as an
    # increase. IEEE Table 3 itself remains the absolute-change screening rule.
    table3_delta_ratio_all = evidence.get("table3_max_increase_ratio_all", np.nan)
    table4_rate_ratio_all = evidence.get("table4_max_ratio_all", np.nan)

    continuous_candidates = []
    if np.isfinite(table2_concentration_ratio_all):
        continuous_candidates.append((float(table2_concentration_ratio_all), "TABLE_2_95TH_CONCENTRATION", None))
    if np.isfinite(table3_delta_ratio_all):
        continuous_candidates.append((float(table3_delta_ratio_all), "TABLE_3_95TH_DELTA", None))
    if np.isfinite(table4_rate_ratio_all):
        continuous_candidates.append((float(table4_rate_ratio_all), "TABLE_4_95TH_RATE", None))
    if continuous_candidates:
        continuous_ratio, continuous_basis, continuous_gas = max(
            continuous_candidates, key=lambda x: x[0]
        )
    elif np.isfinite(table1_concentration_ratio_all):
        continuous_ratio, continuous_basis, continuous_gas = (
            float(table1_concentration_ratio_all), "TABLE_1_90TH_CONCENTRATION", None
        )
    else:
        continuous_ratio, continuous_basis, continuous_gas = np.nan, "NO_CONTINUOUS_EVIDENCE", None

    result = {"ieee_dga_status":int(status),"ieee_dga_status_label":STATUS_LABELS[status],"severity_label_text":STATUS_LABELS[status],"severity_source":STANDARD,"severity_score_type":"IEEE_ORDINAL_STATUS_PLUS_STANDARD_EVIDENCE_VECTOR","severity_is_not_a_health_score":True,"severity_is_failure_probability":False,"severity_composite_weighted":False,"severity_uses_manual_weights":False,"severity_manual_weights":False,"severity_weighted_sum_used":False,"severity_anomaly_used":False,"severity_nei_used":False,"severity_inference_stage":"IEEE_C57_104_2019_RULE_ENGINE","ieee_dga_status_description":STATUS_DESCRIPTIONS[status],"ieee_dga_status_reason":" | ".join(basis),"ieee_recommended_action":STATUS_ACTIONS[status],"ieee_confirmation_required":bool(status in {2,3}),"ieee_o2_n2_ratio":ratio,"ieee_o2_n2_section":selected_section,"ieee_o2_n2_unknown":not np.isfinite(ratio),"ieee_transformer_age_years":age,"ieee_age_bucket":age_bucket,"ieee_table1_exceeding_gases":evidence["table1_exceeding_gases"],"ieee_table2_exceeding_gases":evidence["table2_exceeding_gases"],"ieee_table3_exceeding_gases":evidence["table3_exceeding_gases"],"ieee_table4_exceeding_gases":evidence["table4_exceeding_gases"],"ieee_table1_max_exceedance_ratio":evidence["table1_max_ratio"],"ieee_table2_max_exceedance_ratio":evidence["table2_max_ratio"],"ieee_table3_max_exceedance_ratio":evidence["table3_max_ratio"],"ieee_table4_max_exceedance_ratio":evidence["table4_max_ratio"],"ieee_table1_concentration_ratio_all":float(table1_concentration_ratio_all) if np.isfinite(table1_concentration_ratio_all) else np.nan,"ieee_table2_concentration_ratio_all":float(table2_concentration_ratio_all) if np.isfinite(table2_concentration_ratio_all) else np.nan,"ieee_table3_delta_ratio_all":float(table3_delta_ratio_all) if np.isfinite(table3_delta_ratio_all) else np.nan,"ieee_table4_rate_ratio_all":float(table4_rate_ratio_all) if np.isfinite(table4_rate_ratio_all) else np.nan,"ieee_continuous_evidence_ratio":float(continuous_ratio) if np.isfinite(continuous_ratio) else np.nan,"ieee_continuous_evidence_basis":continuous_basis,"ieee_continuous_evidence_label":"DGA deterioration evidence ratio","ieee_continuous_evidence_is_score":False,"ieee_continuous_evidence_is_probability":False,"ieee_continuous_evidence_reference":"IEEE_TABLE_THRESHOLD_RATIO; per-sample evidence trajectory, not a weighted severity score","ieee_max_standardized_exceedance":max_standardized_exceedance,"ieee_max_status3_standardized_exceedance":concentration_status3_exceedance,"ieee_concentration_exceedance_ratio":float(concentration_exceedance),"ieee_delta_exceedance_ratio":float(delta_exceedance),"ieee_rate_exceedance_ratio":float(rate_exceedance),"ieee_standard_trigger_count":int(standard_trigger_count),"ieee_table3_available":evidence["table3_available"],"ieee_table4_available":evidence["table4_available"],"ieee_delta_available":evidence["table3_available"],"ieee_rate_available":evidence["table4_available"],"ieee_rate_window_points":int(evidence["rate_window"]["points"]) if evidence.get("rate_window") else 0,"ieee_rate_span_months":float(evidence["rate_window"]["span_months"]) if evidence.get("rate_window") else np.nan,"ieee_rate_all_window_points_below_table1":evidence["rate_all_window_points_below_table1"],"ieee_delta":evidence.get("deltas",{}),"ieee_gas_rate_ppm_per_year":evidence.get("rates",{}),"ieee_tdcg_ppm":_numeric_tdcg(values),"ieee_mandatory_gas_complete":complete,"ieee_status_basis":basis,"ieee_standard_evidence_vector":{"status":int(status),"max_standardized_exceedance":float(max_standardized_exceedance),"max_status3_standardized_exceedance":float(concentration_status3_exceedance),"delta_exceedance_flag":int(bool(evidence["table3_exceeding_gases"])),"standard_trigger_count":int(standard_trigger_count)},"iec_60599_standard":IEC_STANDARD,"iec_60599_ratio_available":iec["available_ratio_count"] >= 1,"iec_60599_ratio_count":int(iec["available_ratio_count"]),"iec_60599_ratios":iec["ratios"],"iec_60599_interpretation_flags":iec["interpretation_flags"],"iec_60599_overrides_ieee_status":False,"severity_engine_version":"IEEE_C57_104_2019_SEPARATE_CONCENTRATION_DELTA_RATE_EVIDENCE_v5"}
    return result

def apply_severity(df, nei_reference=None):
    logger.debug("apply_severity: input shape=%s", df.shape if df is not None else None)
    if df is None or df.empty:
        logger.debug("apply_severity: input is None or empty, returning copy/empty")
    ...
    logger.debug("apply_severity: after cleaning and sorting shape=%s", out.shape)
    result_rows = []
    ...
    logger.debug("apply_severity: evaluated %d rows", len(result_df))
    ...
    if "ieee_dga_status" in out.columns:
        logger.debug("IEEE C57.104-2019 severity distribution: %s", out["ieee_dga_status"].value_counts().sort_index().to_dict())
    logger.debug("apply_severity: output shape=%s", out.shape)
    return out
