...
# ============================================================
# STUDENT MODEL TRAINING FALLBACK
# ============================================================

def _train_student_model_from_weak_labels(
    ...
    if df_filtered.empty:
        raise ValueError(
            "No confident weak labels available."
        )
...
def _load_or_fit_anomaly(
    ...
    if ANOMALY_MODEL_PATH.exists():
        ...
        try:
            ...
            logger.exception(
                "Stored anomaly model could not be used; fitting a new one."
            )

    # Initial bootstrap case only.
    ...
    try:
        ...
        logger.exception(
            "Could not save anomaly ensemble."
        )
...
def _trim_row(
    ...
    return {
        key: row_dict[key]
        for key in ROW_EXPORT_FIELDS
        if key in row_dict
    }


# ============================================================
# STUDENT VS TRADITIONAL
# ============================================================

def build_student_traditional_comparison(
    df: pd.DataFrame,
) -> pd.DataFrame:
        ...
        rows.append(
            {
                ...
                "latest_student_fault": str(
                    latest.get(
                        "student_fault_label",
                        "ABSTAIN",
                    )
                ),
                "latest_traditional_fault": str(
                    latest.get(
                        "consensus_fault_traditional",
                        "ABSTAIN",
                    )
                ),
                "latest_severity_score": float(
                    latest.get(
                        "severity_score",
                        0.0,
                    )
        ...
        predictions.append(
            {
                ...
                "severity": cfg.SEVERITY_TO_UI.get(
                    severity_label,
                    severity_label,
                ),
                "fault_type": row.get(
        ...
        transformer_summary.append(
            {
                ...
                "priority_label": cfg.SEVERITY_TO_UI.get(
                    severity_label,
                    severity_label,
                ),
...
def process_dataframe(
    ...
    try:
        ...
        from dataset_accumulator import (
        ...
        # 6. Student model
        #
        # Load a persistent trained model whenever possible.
        # Train only when no artifact exists.
        # ----------------------------------------------------
        ...
        if (
            student_model is None
        ):

            logger.warning(
                "No saved student model. "
                "Training bootstrap student model from current weak labels."
            )
            ...
            (
                ...
                _train_student_model_from_weak_labels(
                    df_weak
                )
            )
        ...
        # ----------------------------------------------------
        # 7. Preserve traditional fault before fusion.
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # 10. IEEE severity/status
        #
        # Anomaly is deliberately NOT used to change IEEE
        # DGA Status.
        # ----------------------------------------------------
        ...
        comparison_df = (
            build_student_traditional_comparison(
                df_labeled
            )
        )
        ...
        # ----------------------------------------------------
        # 15. Background experiments
        # ----------------------------------------------------

        threading.Thread(
            target=_run_experiments_background,
            daemon=True,
        ).start()

        logger.info(
            "DGA processing complete in %.1fs.",
            time.time()
            - start_time,
        )
        ...
        logger.exception(
            "Fatal error in process_dataframe."
        )
