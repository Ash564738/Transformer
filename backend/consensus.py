...
def _o2_n2_ratio(row):
    ...
    o2 = _gas(row, "o2")
    n2 = _gas(row, "n2")
    if np.isfinite(o2) and np.isfinite(n2) and n2 > 0: return o2 / n2
    return np.nan

def is_below_l1(row):
    for gas in cfg.SEVERITY_REQUIRED_GASES:
        if not np.isfinite(_gas(row, gas)): return False
    ratio = _o2_n2_ratio(row)
    if not np.isfinite(ratio): return False
...
def _select_fine_fault(votes, winning_group, selected_methods=None):
    ...
    if not counts: return winning_group
    ranked = sorted(counts.items(), key=lambda item: (-item[1], item[0]))
    if len(ranked) > 1 and ranked[0][1] == ranked[1][1]: return ABSTAIN
    return ranked[0][0]
...
def aggregate_votes_with_row(row, votes, selected_methods):
    ...
    if not concrete:
        ...
        if is_below_l1(row): return "NORMAL", ["NORMAL"], False
...
def make_vote_frame(df, methods=None):
    ...
    if unknown: raise ValueError(f"Unknown diagnostic methods: {sorted(unknown)}")
    if not selected: raise ValueError("At least one diagnostic method is required.")
    rows = []
    ...
    return pd.DataFrame(rows, index=df.index, columns=cfg.DIAGNOSTIC_METHODS)

def consensus_from_vote_frame(df, vote_frame, selected_methods=None):
    selected = list(selected_methods or vote_frame.columns)
    if len(df) != len(vote_frame): raise ValueError("df and vote_frame length mismatch.")
    out = df.copy()
...
def apply_consensus(df, methods=None):
    ...
    return consensus_from_vote_frame(calculated, votes, selected)
...
def apply_consensus_from_existing_diagnostics(df, methods):
    ...
    return consensus_from_vote_frame(df, votes, selected)
