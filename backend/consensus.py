...
def _gas(row, name):
    ...
    logger.debug("_gas: %s=%s", name, value)
    return value

def normalize_fault(label):
    logger.debug("normalize_fault input=%r", label)
    ...
    logger.debug("normalize_fault output=%s", ABSTAIN)
    return ABSTAIN
...
def unify_fault(label):
    ...
    logger.debug("unify_fault: fine=%s output=%s", fine, ABSTAIN)
    return ABSTAIN

def _safe_ratio(value):
    try: value = float(value)
    except (TypeError, ValueError): return np.nan
    result = value if np.isfinite(value) and value >= 0 else np.nan
    logger.debug("_safe_ratio: output=%s", result)
    return result
...
def _safe_age(row):
    for column in ("transformer_age_years", "age_years"):
        ...
        if np.isfinite(value):
            logger.debug("_safe_age: %s=%s", column, value)
            return value
    ...
    if np.isfinite(sample_year) and np.isfinite(energized_year) and sample_year >= energized_year:
        ...
        logger.debug("_safe_age: computed=%s", age)
        return age
    logger.debug("_safe_age: unknown")
    return np.nan
...
def _o2_n2_ratio(row):
    for column in ("o2_n2_ratio", "o2n2_ratio"):
        value = _safe_ratio(row.get(column, np.nan))
        if np.isfinite(value):
            logger.debug("_o2_n2_ratio: %s=%s", column, value)
            return value
    o2 = _gas(row, "o2")
    n2 = _gas(row, "n2")
    if np.isfinite(o2) and np.isfinite(n2) and n2 > 0:
        ratio = o2 / n2
        logger.debug("_o2_n2_ratio: computed=%s", ratio)
        return ratio
    logger.debug("_o2_n2_ratio: not available")
    return np.nan

def is_below_l1(row):
    for gas in cfg.SEVERITY_REQUIRED_GASES:
        if not np.isfinite(_gas(row, gas)):
            logger.debug("is_below_l1: missing gas %s", gas)
            return False
    ratio = _o2_n2_ratio(row)
    if not np.isfinite(ratio):
        logger.debug("is_below_l1: missing o2/n2 ratio")
        return False
    section = "LE_0_2" if ratio <= 0.2 else "GT_0_2"
    age_bucket = _age_bucket(_safe_age(row))
    limits = cfg.TABLE_1_90TH[section][age_bucket]
    logger.debug("is_below_l1: ratio=%.4f section=%s age_bucket=%s", ratio, section, age_bucket)
...
def diagnostic_agreement_ratio(votes, selected_group, selected_methods=None):
    ...
    if not groups:
        logger.debug("diagnostic_agreement_ratio: no groups")
        return 0.0
    result = round(100.0 * sum(g == selected_group for g in groups) / len(groups), 1)
    logger.debug("diagnostic_agreement_ratio: selected_group=%s groups=%s result=%s", selected_group, groups, result)
    return result
...
def _active_votes(votes, selected_methods=None):
    ...
    logger.debug("_active_votes: methods=%s active=%s", methods, out)
    return out
...
def _group_vote_counts(votes, selected_methods=None):
    ...
    logger.debug("_group_vote_counts: counts=%s", counts)
    return counts
...
def _select_fine_fault(votes, winning_group, selected_methods=None):
    ...
    if not counts:
        logger.debug("_select_fine_fault: no counts; winning_group=%s", winning_group)
        return winning_group
    ranked = sorted(counts.items(), key=lambda item: (-item[1], item[0]))
    if len(ranked) > 1 and ranked[0][1] == ranked[1][1]:
        logger.debug("_select_fine_fault: tie -> ABSTAIN")
        return ABSTAIN
    result = ranked[0][0]
    logger.debug("_select_fine_fault: result=%s", result)
    return result

def aggregate_votes_with_row(row, votes, selected_methods):
    active = _active_votes(votes, selected_methods)
    logger.debug("aggregate_votes_with_row: active=%s", active)
    if not active:
    ...
    logger.debug("aggregate_votes_with_row: concrete=%s mixed_count=%s", concrete, mixed_count)
    if not concrete:
    ...
    logger.debug("aggregate_votes_with_row: winning_group=%s fine=%s", winning_group, fine)
    return fine, [winning_group], False

def _apply_traditional_methods(df):
    logger.info("_apply_traditional_methods: input shape=%s", df.shape)
    ...
    logger.info("_apply_traditional_methods: output shape=%s", out.shape)
    return out
...
def make_vote_frame(df, methods=None):
    ...
    logger.info("make_vote_frame: rows=%d methods=%s", len(df), selected)
    rows = []
    ...
    logger.info("make_vote_frame: vote_frame shape=%s", vote_frame.shape)
    return vote_frame
...
def consensus_from_vote_frame(df, vote_frame, selected_methods=None):
    ...
    logger.info("consensus_from_vote_frame: rows=%d methods=%s", len(df), selected)
    out = df.copy()
    ...
    for position, (_, row) in enumerate(df.iterrows()):
        ...
        logger.debug(
            "consensus_from_vote_frame row=%d active=%d fault=%s group=%s conflict=%s votes=%s",
            position, active, fault, group, conflict, votes,
        )
    ...
    logger.info("consensus_from_vote_frame: done rows=%d", len(out))
    return out

def apply_consensus(df, methods=None):
    logger.info("apply_consensus: input shape=%s", df.shape)
    calculated = _apply_traditional_methods(df)
    selected = list(methods or cfg.DIAGNOSTIC_METHODS)
    logger.info("apply_consensus: selected_methods=%s", selected)
    votes = make_vote_frame(calculated, selected)
    result = consensus_from_vote_frame(calculated, votes, selected)
    logger.info("apply_consensus: output shape=%s", result.shape)
    return result
...
def apply_consensus_from_existing_diagnostics(df, methods):
    ...
    logger.info("apply_consensus_from_existing_diagnostics: rows=%d methods=%s", len(df), selected)
    votes = make_vote_frame(df, selected)
    result = consensus_from_vote_frame(df, votes, selected)
    logger.info("apply_consensus_from_existing_diagnostics: output shape=%s", result.shape)
    return result
...
def diagnostic_method_summary(df):
    logger.info("diagnostic_method_summary: rows=%d", len(df))
    rows = []
...
def pairwise_label_agreement(df, methods=None):
    selected = list(methods or cfg.DIAGNOSTIC_METHODS)
    logger.info("pairwise_label_agreement: rows=%d methods=%s", len(df), selected)
    rows = []
...
def evaluate_method_labels(y_true, predicted_labels, allowed_labels):
    ...
    logger.info("evaluate_method_labels: truth=%d pred=%d allowed=%s", len(truth), len(pred), sorted(allowed))
    ...
    if n_eval == 0:
        logger.info("evaluate_method_labels: n_eval=0")
        return {
