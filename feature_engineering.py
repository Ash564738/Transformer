...
def sort_and_deduplicate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = (
        out.sort_values(["transformer_id", "sample_day"])
...
def impute_optional_context_by_transformer(df: pd.DataFrame) -> pd.DataFrame:
    """
    Chỉ fill context columns; KHÔNG fill core gases, KHÔNG fill temp/water.
    temp/water giữ missing nguyên trạng vì user yêu cầu không tự fill.
    """
    out = df.copy()
...
def add_tdcg(df: pd.DataFrame) -> pd.DataFrame:
    """
    tdcg_raw = cột lab gốc (nếu có)
    tdcg_recalc = sum combustible gases
    tdcg = final feature value ưu tiên raw, fallback recalc
    """
    out = df.copy()
...
def add_rolling_features(df: pd.DataFrame, value_cols: List[str]) -> pd.DataFrame:
    """
    History-only rolling: chỉ dùng quá khứ, không include current row
    """
    out = df.copy()
    new_cols = {}
...
def add_ewm_features(df: pd.DataFrame, value_cols: List[str]) -> pd.DataFrame:
    """
    History-only EWMA trên shift(1)
    """
    out = df.copy()
