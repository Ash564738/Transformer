...
import { ieeeStatusToRiskStatus, statusFromSummary } from "@/lib/severity";
...
function latestRowMap(rows: DgaRow[]) {
  ...
  for (const row of rows) {
    ...
    if (
      !current ||
      new Date(row.sample_day).getTime() > new Date(current.sample_day).getTime()
    ) {
...
export function RankingTable({
  ...
  const filteredRows = useMemo(() => {
    ...
    if (statusFilter !== "All") {
      list = list.filter(
        (summary) => statusFromSummary(summary) === statusFilter
      );
    }
  ...
  const sortedRows = useMemo(() => {
    ...
    return list.sort((a, b) => {
      switch (sortColumn) {
        ...
        case "status":
          return (
            (statusOrder.indexOf(statusFromSummary(a)) -
              statusOrder.indexOf(statusFromSummary(b))) *
            dir
          );
        case "fault":
          return (
            (a.fault_type || "").localeCompare(b.fault_type || "") * dir
          );
        case "evidence":
          return (
            (Number(a.current_status3_standardized_exceedance ?? 0) -
              Number(b.current_status3_standardized_exceedance ?? 0)) *
            dir
          );
        case "topGas":
          return (
            (Number(topGas(latestRows.get(a.transformer_id))?.value ?? -Infinity) -
              Number(topGas(latestRows.get(b.transformer_id))?.value ?? -Infinity)) *
            dir
          );
        case "date":
          return (
            (new Date(a.latest_sample_day).getTime() -
              new Date(b.latest_sample_day).getTime()) *
            dir
          );
        default:
          return 0;
  ...
  function handleSort(column: SortColumn) {
    ...
    setSortDirection(
      column === "rank" || column === "status" ? "asc" : "desc"
    );
  }

  function SortIcon({ column }: { column: SortColumn }) {
    if (column !== sortColumn) {
      return <ArrowUpDown className="h-3 w-3 text-teal-300" />;
    }
  ...
  return (
    <div className="space-y-4">
      {!limit && (
        <div className="flex flex-wrap items-end justify-end gap-3">
          ...
          <select
            ...
            className="h-10 rounded-lg border border-teal-200 bg-white px-3 text-sm text-teal-800"
          >
            {stations.map((item) => (
          ...
          <select
            value={faultFilter}
            onChange={(event) => setFaultFilter(event.target.value)}
            className="h-10 rounded-lg border border-teal-200 bg-white px-3 text-sm text-teal-800"
          >
            {faultTypes.map((fault) => (
              <option key={fault} value={fault}>
                {fault === "All" ? "All Faults" : fault}
              </option>
            ))}
          </select>
          ...
          <select
            ...
            className="h-10 rounded-lg border border-teal-200 bg-white px-3 text-sm text-teal-800"
          >
            <option value="All">All IEEE Statuses</option>
            <option value="High">Status 3</option>
            <option value="Watch">Status 2</option>
            <option value="Normal">Status 1</option>
      ...
      <div className="rounded-lg border border-teal-100 bg-teal-50 px-3 py-2 text-[11px] leading-5 text-teal-700">
        Fleet rank is a relative, unweighted lexicographic ordering. It is not
        a probability or weighted health score. S3 evidence is taken from the
        latest sample; history is shown separately.
      </div>
          ...
          <table className="w-full min-w-[1280px] text-left text-sm">
            ...
            <tbody>
              {displayRows.map((summary) => {
                ...
                const isStatus3 = ieeeStatus === "High";
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-cream-200 last:border-0 hover:bg-cream-50 ${
                      isStatus3 ? "bg-orange-50/30" : ""
                    }`}
                  >
                    <td className="px-4 py-3 font-mono font-semibold text-teal-700">
                      {summary.maintenance_rank ?? summary.rank}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <StatusBadge status={ieeeStatusToRiskStatus(summary.ieee_status)} />
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 font-mono text-teal-700">
                      {summary.current_status3_standardized_exceedance != null
                        ? `${formatNumber(
                            summary.current_status3_standardized_exceedance,
                            2
                          )}×`
                        : "—"}
                    </td>
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-teal-500">
      ...
      <span className="text-teal-400">
        Maintenance rank is separate and ordered by explicit evidence.
      </span>
    </div>
  );
}
