...
import { ROGERS_3D_ZONES } from "./rogers3d-zones";
import { buildZoneTraces } from "./build-zone-traces";
...
interface Props {
  ...
  r2: number;
  r3: number;
  fault?: string;   // ví dụ: "THERMAL FAULT > 700°C (T3)"
}

export function Ratio3DChart({ r1, r2, r3, fault }: Props) {
  const pointValid =
  ...
  const traces = useMemo(() => {
    const zoneTraces = buildZoneTraces(ROGERS_3D_ZONES, faultCode);
      ...
      ? [
          {
            ...
            showlegend: false,
            hoverinfo: "text",
            hovertext: `R1 (CH₄/H₂): ${r1.toFixed(3)}<br>R2 (C₂H₂/C₂H₄): ${r2.toFixed(3)}<br>R3 (C₂H₄/C₂H₆): ${r3.toFixed(3)}`,
          },
        ]
      : [];
  ...
  const legendItems = ROGERS_3D_ZONES.map((zone) => ({
    name: zone.name,
    desc: zone.desc,
    color: zone.color,
  }));
