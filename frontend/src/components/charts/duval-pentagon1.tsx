...
const V = {
  H2: [0, 30] as const,
  C2H2: [28.5, 9.3] as const,
  C2H4: [17.625, -24.3] as const,
  CH4: [-17.625, -24.3] as const,
  C2H6: [-28.5, 9.3] as const,
};

const ZONES_P1: Record<string, [number, number][]> = {
  ...
  T3: [[0, -2.25], [18.225, -22.5], [17.625, -24.3], [0.75, -24.3], [-4.5, -3]],
  T2: [[-4.5, -3], [0.75, -24.3], [-16.875, -24.3]],
  T1: [[-4.5, -3], [-16.875, -24.3], [-17.625, -24.3], [-26.25, 2.325], [0, 1.125], [0, -2.25]],
...
function diagnoseP1(cent: { x: number; y: number }) {
...
export function DuvalPentagon1Svg({
  ...
  const zoneEls = useMemo(() => {
    return Object.entries(ZONES_P1).map(([zone, poly]) => {
          ...
          <g onMouseEnter={() => setShowTooltip(true)} onMouseLeave={() => setShowTooltip(false)}>
            ...
            {showTooltip && (
              <>
                ...
                <text x={tx + tooltipW/2} y={ty + 1.8} textAnchor="middle" fontSize="1.2" fill="darkred" fontWeight="bold">
                  H₂: {pH2.toFixed(1)}%
                </text>
                <text x={tx + tooltipW/2} y={ty + 2.8} textAnchor="middle" fontSize="1.2" fill="darkred" fontWeight="bold">
                  CH₄: {pCH4.toFixed(1)}%
                </text>
                <text x={tx + tooltipW/2} y={ty + 3.8} textAnchor="middle" fontSize="1.2" fill="darkred" fontWeight="bold">
                  C₂H₆: {pC2H6.toFixed(1)}%
                </text>
                <text x={tx + tooltipW/2} y={ty + 4.8} textAnchor="middle" fontSize="1.2" fill="darkred" fontWeight="bold">
                  C₂H₄: {pC2H4.toFixed(1)}%
                </text>
                <text x={tx + tooltipW/2} y={ty + 5.8} textAnchor="middle" fontSize="1.2" fill="darkred" fontWeight="bold">
                  C₂H₂: {pC2H2.toFixed(1)}%
                </text>
                <text x={tx + tooltipW/2} y={ty + 7} textAnchor="middle" fontSize="1.2" fill="red" fontWeight="bold">
                  Fault: {backendFault || fault}
                </text>
              </>
            )}
          </g>
